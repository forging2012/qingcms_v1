...
class PubVideo {
	...
	public function paramUrl() {
		...
		if (preg_match ( "/(youku.com|youtube.com|ku6.com|sohu.com|mofile.com|sina.com.cn|tudou.com)$/i", $parseLink ['host'], $hosts )) {
			...
			$return ['boolen'] = 0;
			$return ['message'] = '不支持该视频网站';
		}
		
		$flashinfo = $this->_video_getflashinfo ( $link, $hosts [1] );
		// dump($flashinfo);
		$return ['content'] = $flashinfo ['title'] . $link;
		/*
		 * $flashinfo title img flashvar host source
		 */
		$flashinfo ['host'] = $hosts [1];
		$flashinfo ['source'] = $link;
		$return ['info'] = serialize ( $flashinfo ); // 序列化 - 生成一个值可储存
	...
	private function _video_getflashinfo($link, $host) {
		...
		if ('youku.com' == $host) {
			...
			// preg_match("/<span
			// class=\"s_pic\">(.*?)<\/span>/i",$content,$img);
